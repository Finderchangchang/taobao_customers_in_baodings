                ...
                for (int i = 0; i < grantResults.length; ++i) {
                    if (grantResults[i] == PackageManager.PERMISSION_DENIED) {
                        ...
                        if (!ActivityCompat.shouldShowRequestPermissionRationale(this, permissions[i])) {
                            //【用户选择了不再提示按钮，或者系统默认不在提示（如MIUI）。" +"引导用户到应用设置页去手动授权,注意提示用户具体需要哪些权限】
                            new android.app.AlertDialog.Builder(BaseActivity.this).setTitle("权限请求")//设置对话框标题
                                    .setMessage("" +
                                            "获取相关权限失败:将导致部分功能无法正常使用，需要到设置页面手动授权")//设置显示的内容
    ...
    public void requestPermissions(final Context context, final String[] permissions,
        ...
        for (String permission : permissions) {
            if (ContextCompat.checkSelfPermission(context, permission) == PackageManager.PERMISSION_DENIED) {
                isAllGranted = false;//【用户曾经拒绝过你的请求，所以这次发起请求时解释一下】
                if (ActivityCompat.shouldShowRequestPermissionRationale((Activity) context, permission)) {
                    new android.app.AlertDialog.Builder(BaseActivity.this).setTitle("权限请求")//设置对话框标题
                            .setMessage("" +
                                    "您好，需要获取您的位置信息请允许，否则将影响部分功能的正常使用。")//设置显示的内容
